...
```bash
npx datadog-connect@latest mcp
```

これだけで Claude Code と Datadog が繋がります。

> **うまくいかない場合**: `npx` のキャッシュが古いバージョンを使う場合があります。`@latest` を付けると最新版が確実に使われます。
...
> **初めての方におすすめ**: `npx datadog-connect@latest plan --preset minimal` で、モニター + ダッシュボードだけの最小セットアップを 5 分で体験できます。
...
| コマンド | 説明 |
|---------|------|
| `npx datadog-connect@latest setup` | 対話形式でセットアップ |
| `npx datadog-connect@latest plan` | 設定プランを生成して表示 |
| `npx datadog-connect@latest resume` | 前回の途中から再開 |
| `npx datadog-connect@latest rollback` | 作成した設定を元に戻す |
| `npx datadog-connect@latest mcp` | Claude Code と Datadog を接続 |
| `npx datadog-connect@latest sessions` | 過去のセッション一覧を表示 |

### 便利なオプション

```bash
# 5分で完了するクイックスタート
npx datadog-connect@latest plan --preset minimal

# JSON形式で出力（スクリプト連携用）
npx datadog-connect@latest setup --format json

# 削除前にプレビュー（安全確認）
npx datadog-connect@latest rollback --dry-run

# 過去5件のセッションを表示
npx datadog-connect@latest sessions --limit 5
```
